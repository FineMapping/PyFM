...
class Configurations:
    def __init__(self,**kwarg):
        ...
        self.m = kwarg['m']
        self.score_config = kwarg['score_config'] # method to calculate score for a config
...
class AllConfigurations(Configurations):
    ...
    def __init__(self,**kwarg):
        ...
        self.config = np.arange(self.n_causal)+1 # list of indices of selected variants
    ...
    def next(self):
        ...
        """
        i = self.n_causal # 1-based index at which config will change, i.e index of p-1 -> p
        ...
        if (i == 0):
            ...
            self.config[i-1:] = self.config[i-1]+1 + np.arange(self.n_causal - i+1)
            self.current_score = self.score_config(self.config)
...
def ConfigurationsFactory(configs_impl):
    if configs_impl not in CONFIGS_IMPL_MAP:
        raise ValueError(f"Must choose one configurations implementation from {CONFIGS_IMPL_MAP.keys()}")
    else:
