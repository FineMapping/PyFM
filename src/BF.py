...
def outputBF(data, n, pve_for_prior, prior_values, e, max_causal, configs_method, approx_bf, filename):

    count = 0
    # TODO: consider putting u in data
    u = (data.pve.values if data.pve is not None else np.ones(data.m)) * (n if approx_bf else (n-1))
    for n_causal in range(1,max_causal+1):
        config_iter = ConfigurationsFactory(configs_method)(n_causal = n_causal, m = data.m,\
                        score_config = lambda t: config_BF(t, data, n, pve_for_prior, prior_values, e, n_causal, approx_bf, u))

        config_iter.search(lambda iter: print("config:",iter.config,"score:",iter.current_score))
...
def config_BF(t, data, n, pve_for_prior, prior_values, e, n_causal, approx_bf, u):
    ...
    mean_logBF = (max(logBFs)+ log(exp(np.array(logBFs) - max(logBFs)).mean())) / log(10)
    return mean_logBF
