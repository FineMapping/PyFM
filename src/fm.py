...
def parse_args():
    ...
    parser.add_argument("-z", "--zfile",help = 'Marginal test statistics with 3 columns - rsid, standardized effect size, (optional) variance of variant', required=True)
    parser.add_argument("-r", "--rfile",help='LD scores matrix', required=True)
    parser.add_argument("-o", "--outdir", required=True)
    parser.add_argument("-c", "--max-causal",help='Maximum number of causal variants (inclusive)', default= 3)
    ...
    parser.add_argument("-e", "--epsilon", help = "the value added to the diagonal of the correlation matrix.", default= 0)
    parser.add_argument("--approx-bf", help = "Calculate approx BF (recommended for Binary trait) instead of exact BF (recommended for Quant. trait)", action='store_true')
    parser.add_argument("--configs-method", help = f"choose one causal configurations exploration method from {CONFIGS_IMPL_MAP.keys()}", default = "AllConfigurations")
    ...
    return data, n, pve_for_prior, prior_values, e, max_causal, configs_method, approx_bf, outdir

if __name__ == "__main__":
    data, n, pve_for_prior, prior_values, e, max_causal, configs_method, approx_bf, outdir = parse_args()
    BF = outputBF(data, n, pve_for_prior, prior_values, e, max_causal, configs_method, approx_bf, os.path.join(outdir, 'BF.tsv'))
